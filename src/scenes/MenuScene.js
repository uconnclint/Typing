import {
  GameConfig, savePrefs, TRACKS,
  getSelectedTrackIndex, setSelectedTrackIndex,
...
export default class MenuScene extends Phaser.Scene {
  ...
  create(){
    ...
    this.muteText = this.add.text(width-60, 20, (localStorage.getItem('kr_muted')==='1')?'🔇':'🔊', {fontSize:'28px'})
      ...
      .on('pointerdown', () => { SFX.setMuted(!SFX.muted); this.sound.mute = SFX.muted; this.muteText.setText(SFX.muted ? '🔇' : '🔊'); });

    // Init UI + audio
    this._refresh();
    this.sound.mute = (localStorage.getItem('kr_muted')==='1');
    this._changeTrack(0); // play current selection
    this._changeChar(0);  // set current character
  }

  _fitPreview(){
    // scale preview to ~64px tall without blurring pixel art
  ...
  _changeTrack(delta){
    ...
    snd.play(); this.music = snd;
    this.sound.mute = SFX.muted;
  }

  _button(x,y,w,h,label,cb){
    const bg = this.add.rectangle(x,y,w,h,0x2a334d).setOrigin(0);
    const txt = this.add.text(x+w/2, y+h/2, label, { fontFamily:'"Press Start 2P"', fontSize:'14px', color:'#e6f3ff' }).setOrigin(0.5);
